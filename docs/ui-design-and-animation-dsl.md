...
Child 的 vertical scroll indicator 轨道跟随 Pager 输出的 managed inset：顶部避让 Header/TabBar 占位，底部使用当前安全区与可见底部遮挡的最大值。Pager 接管后关闭 UIKit 自动 scroll indicator inset 调整，避免 safe area 或 bar 避让被重复计算。内容滚到底部时，indicator thumb 应停在安全区或可见底部遮挡上方，而不是沿用 child 接管前的旧自动调整值。

Child content bottom inset 允许业务或第三方刷新/加载更多组件在 Pager 基线之上追加空间。Pager 因 `contentSize`、safe area、TabBar 遮挡或短内容补偿重算 managed inset 时，只替换自己的基线，不应抹掉这部分外部追加值；底部 footer、loading 和 no-more 提示应完整显示在可见底部遮挡上方。
...
component CollapsiblePager {
  ...
  layout default {
    ...
    childScrollView.bottomInset >= max(safeArea.bottom, visibleBottomObstruction)
    childScrollView.bottomInset >= viewport.height + pinThreshold - childScrollView.topInset - contentSize.height
    externalBottomFooter.frame.maxY <= pager.bounds.maxY - max(safeArea.bottom, visibleBottomObstruction)
...
state pagingProgrammatic {
  ...
  if trigger == tabTap then normalize targetChild to current pinAnchorY unless current and target are both pinned
...
state pagingCompleted {
  ...
  clear pendingPageRequestSource
  if source == tabTap then keep current pinAnchorY and align targetChild unless current and target are both pinned
  if source == gesture and pendingTargetIndex == targetIndex then adopt pinAnchorY from targetChild.contentOffsetY
  if source == gesture and pendingTargetIndex != targetIndex then align targetChild to current pinAnchor when needed
  if source == api then align targetChild to current pinAnchor when needed
  restore headerHost by targetChild vertical state
}
...
state pagingGestureSettledIntermediate {
  trigger pageContainer.completion
  guard pendingPageRequestSource == gesture
  guard completion.targetIndex != pendingSelectedIndex
  guard PagePosition == stable(completion.targetIndex)
  commit selectedIndex = completion.targetIndex
  clear pendingSelectedIndex
  clear pendingPageRequestSource
  restore headerHost by pinAnchor
}

state pagingCompletionDeferredDuringStartup {
  trigger pageContainer.completion
  guard isStartingAnimatedPageScroll == true
  store deferredCompletionTarget = completion.targetIndex
  keep selectedIndex = sourceIndex
  keep pendingSelectedIndex = latestTargetIndex
  wait until PagePosition == stable(deferredCompletionTarget)
  consume deferredCompletionTarget
  commit selectedIndex = deferredCompletionTarget
}

state pagingSettledPendingRetap {
  trigger tabTap(targetIndex)
  guard pendingSelectedIndex == targetIndex
  guard PagePosition == stable(targetIndex)
  commit selectedIndex = targetIndex
  clear pendingSelectedIndex
  clear pendingPageRequestSource
}

state pagingDuplicatePendingRetap {
  trigger tabTap(targetIndex)
  guard pendingSelectedIndex == targetIndex
  guard PagePosition != stable(targetIndex)
  keep selectedIndex = sourceIndex
  keep pendingSelectedIndex = targetIndex
  keep PagePosition
}
...
state pagingCompletionIgnored {
  ...
  guard pendingPageRequestSource != gesture || PagePosition != stable(completion.targetIndex)
  keep selectedIndex = sourceIndex
  keep pendingSelectedIndex = latestTargetIndex
...
当 Header 完全展开、current child 位于 managed top boundary，且手势从 Header 或 TabBar 区域明确向上开始时，视觉应表现为普通列表向上滚动：Header 开始折叠、TabBar 向 pin 位置移动，host 不保留 baseline 以上或以下的普通滚动位移。这个 upward proxy 不显示刷新 reveal，不创建或残留 active `.containerHost`，同一 pan 后续继续向上时仍持续推动 current child，不能只转交第一帧 delta。
...
assert tabTapToUnvisitedPageKeepsPartiallyCollapsedHeader {
  given selectedPage == Long
  and collapseProgress > 0
  and collapseProgress < 1
  when tabTap(Empty)
  then selectedIndex == Empty
  and collapseProgress remains unchanged
  and tabBar.frame.minY remains unchanged
  and emptyScrollView.contentOffset.y == pinAnchorY - managedTopInset
}

assert emptyStateCentersInVisibleContentArea {
  given selectedPage == Empty
  and collapseProgress == 1
  and childScrollView.contentInset.bottom includes shortContentCompensation
  and childScrollView.verticalScrollIndicatorInsets.bottom == bottomObstructionInset
  then emptyState.centerY == visibleContentArea(excludingPinnedHeaderAndTabBar, bottomObstructionInset).centerY
  and emptyState.centerX == childViewport.centerX
}
...
assert statusBarScrollsToTopUsesSingleOwner {
  given collapseProgress < 1
  then containerRefreshHostScrollView.scrollsToTop == true
  and currentChild.scrollView.scrollsToTop == false
  and nonCurrentChild.scrollView.scrollsToTop == false
  and pageContainer.scrollView.scrollsToTop == false
  when statusBarTap
  then containerRefreshHostScrollView handles request
  and currentChild.contentOffsetY animates to expandedTopBoundary when partiallyCollapsed
  given collapseProgress > 0 and collapseProgress < 1
  then containerRefreshHostScrollView.contentOffsetY > -containerRefreshHostScrollView.adjustedContentInsetTop
  and containerRefreshHostScrollView.visualBaseline == neutralBaseline
  and visualFrames remain unchanged
  given collapseProgress == 1
  then containerRefreshHostScrollView.scrollsToTop == false
  and currentChild.scrollView.scrollsToTop == true
  and nonCurrentChild.scrollView.scrollsToTop == false
  and pageContainer.scrollView.scrollsToTop == false
}
...
- 空内容 child：验证 managed inset、外部刷新 reveal，以及空态内容按可见内容区居中，不以 child root view frame 直接作为视觉居中区域。
- 长内容底部上拉：示例 App 的 `Long` child 使用 `Refreshable.loadMoreable` 演示业务侧加载更多，验证 Pager 管理 child inset 后仍能触发底部加载；切换离开 Long 后，未完成的 load-more completion 不应继续追加行或重载离屏列表；Long 已滚到底部并显示 no-more 后，“没有更多数据”应显示在底部 TabBar 可见遮挡上方，切到 Empty 再切回 Long 应恢复 72 行、no-more 状态和 footer 可见位置，不应因程序化恢复底部 `contentOffset` 自动开始加载更多，也不应因为 Long child 被卸载重建而把已加载行数从 72 重置回 60；当当前 Header 已展开或半折叠时，点击 Tab 切回 Long 不能被 no-more footer 的非用户程序化深 offset 立即拉回吸顶底部状态。
...
- `.child` 刷新：业务可在每个 child 的 `pagerScrollView` 上安装局部刷新机制；示例 App 顶部刷新使用系统 `UIRefreshControl` 只是演示选择，不是核心契约。
- 快速点击 Tab：最终停在最后目标。
